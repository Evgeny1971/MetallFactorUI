    ...
    AuthenticationStateProvider authenticationStateProvider)
{
    ...
    public async Task CheckoutAsync(BasketCheckoutInfo checkoutInfo)
    {
        ...
        var buyerId ="1";// = await authenticationStateProvider.GetBuyerIdAsync() ?? throw new InvalidOperationException("User does not have a buyer ID");
        var userName="2";// = await authenticationStateProvider.GetUserNameAsync() ?? throw new InvalidOperationException("User does not have a user name");
